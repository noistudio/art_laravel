...
class Block extends AbstractBlock {
    ...
    public function run() {
        $controller = new \mg\controllers\frontend\Mg();
        if ($this->value == "list") {



            $conditions['$and'][] = array("enable" => 1);

            if (count($this->params['fields'])) {

                foreach ($this->params['fields'] as $name => $arr) {
                    $check = str_replace("last_id", "", $name);
                    if ($check != $name) {
                        $arr['value'] = (int) $arr['value'];
                    }

                    if ($arr['type'] == "=") {
                        $conditions['$and'][] = array($name => $arr['value']);
                    } else if ($arr['type'] == "!=") {
                        $conditions['$and'][] = array($name => array('$ne' => $arr['value']));
                    } else if ($arr['type'] == ">") {
                        $conditions['$and'][] = array($name => array('$gt' => $arr['value']));
                    } else if ($arr['type'] == ">=") {
                        $conditions['$and'][] = array($name => array('$gte' => $arr['value']));
                    } else if ($arr['type'] == "<") {
                        $conditions['$and'][] = array($name => array('$lt' => $arr['value']));
                    } else if ($arr['type'] == "<=") {
                        $conditions['$and'][] = array($name => array('$lte' => $arr['value']));
                    } else if ($arr['type'] == "LIKE") {
                        $conditions['$and'][] = array($name => array('$options' => 'i', '$regex' => $arr['value']));
                    }
                }
            }

            if (count($conditions['$and']) == 1) {
    ...
    public function editPage() {
        $controller = new \mg\controllers\backend\ManageMg();
        if ($this->value == "list") {
            $cur_data = array();
            $data = array();
            $table = CollectionModel::get($this->op);
            $route = "mg";
            $template_fields = "";
            if (count($table['fields'])) {
                ...
                $template_fields = '<a href="/' . $route . '/' . $table['name'] . '/<?php echo $row["last_id"];?>">' . $template_fields . "</a>";
            }
            $data['table'] = $table;
    ...
    foreach ($this->rows as $row) { ?>' . $template_fields . '<?php } }';
            ...
            $data['path_template'] = \core\ManagerConf::getTemplateFolder(true, "frontend") . "plugin/mg/" . $this->op . "_list" . $this->params['postfix_template'] . ".php";
            ...
            $row = \mg\MongoQuery::get($this->op, array("last_id" => (int) $this->value));
            if (is_array($row)) {
                $table = CollectionModel::get($this->op);
                ...
                if (count($table['fields'])) {
                    ...
                    foreach ($table['fields'] as $field_name => $field) {
                        $template_fields .= '<p><?php echo $this->document[' . $field_name . '];  ?></p>';
                    }
                }
                if (!isset($this->params['postfix_template'])) {
                    $this->params['postfix_template'] = "";
                ...
                $data['template'] = $template;
                $data['params'] = $this->params;
                $data['path_template'] = \core\ManagerConf::getTemplateFolder(true, "frontend") . "plugin/mg/" . $this->op . "_one" . $this->params['postfix_template'] . ".php";
    ...
    public function validate() {
        ...
        if ($this->value == "list") {
            ...
            if (isset($this->params['order_field']) and is_string($this->params['order_field'])) {
                if ($this->params['order_field'] == "arrow_sort") {
                    ...
                    if (isset($option_fields[$this->params['order_field']])) {
                        ...
                        $this->addError(__("backend/mg.error_sort"));
                        return null;
                    }
                }
            }
                ...
                foreach ($conditions as $name_field) {
                    ...
                    if ($name_field == "table_limit") {
                        ...
                        if (!(isset($this->params['table_limit']) and is_numeric($this->params['table_limit']) and (int) $this->params['table_limit'] > 0)) {
                            $this->addError(__("backend/mg.error_limit"));
                            return null;
                        }
                        ...
                        if (!(isset($this->params['type_' . $name_field]) and is_string($this->params['type_' . $name_field]) and in_array($this->params['type_' . $name_field], $types_array))) {
                            $this->addError(__("backend/mg.error_type") . " " . $name_field . "!");
                            return null;
                        }
            ...
            if (!is_array($row)) {
                $this->addError(__("backend/mg.error_doc_not_found"));
                return null;
            }
    ...
    public function addPage() {
        ...
        if ($this->value == "list") {
            ...
            $data['table'] = $table;
            $template = '<?php if (count($this->rows)) {
    foreach ($this->rows as $row) { ?>' . $template_fields . '<?php } } echo $this->pages;';
            $template = htmlspecialchars($template);
            $data['template'] = $template;
            $data['params'] = array('postfix_template' => "", 'order_field' => 'arrow_sort');
            $data['block'] = array("id" => '1');

            $data['fields'] = RowModel::editFields($this->op, $data['params'], false, "{param}[val_", "]");
            $data['path_template'] = \core\ManagerConf::getTemplateFolder(true, "frontend") . "plugin/mg/" . $this->op . "_listПОСТФИКСшаблона.php";
    ...
    foreach ($this->rows as $row) { ?>' . $template_fields . '<?php } } ';
                ...
                $data['path_template'] = \core\ManagerConf::getTemplateFolder(true, "frontend") . "plugin/mg/" . $this->op . "_oneПОСТФИКСшаблона.php";
                return $controller->partial_render("add_block_one", $data);
            }
        }
        return "";
