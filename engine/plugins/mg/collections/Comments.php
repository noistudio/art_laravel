...
class Comments extends _DefaultCollection {
    ...
    function __construct($nametable = null, $lang = "null") {
        $nametable = "comments";
...
// build and execute the query
    ...
    public function _delete($condition) {
        ...
        $comments = \mg\MongoQuery::all("comments", $condition);
        ...
        if (isset($comments) and is_array($comments) and count($comments) > 0) {
            foreach ($comments as $comment) {
                if (isset($comment['post']['last_id'])) {
                    $count = \mg\MongoQuery::count("comments", array("enable" => 1, "post.last_id" => (int) $comment['post']['last_id']));
                    $upd = array();
                    $upd['comments'] = $count;

                    \mg\MongoQuery::update($upd, "posts", array('last_id' => (int) $comment['post']['last_id']));
                }
            }
        }
        return $result;
