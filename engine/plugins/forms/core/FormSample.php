...
class FormSample extends Form {
    ...
    private function validateFields() {
        ...
        foreach ($form['fields'] as $name => $field) {
            ...
            if (($field['required'] == 1 and ( !isset($post[$name]) and ! isset($_FILES[$name]) ))) {
                \core\Notify::add("Вы не заполнили поле " . $field['title'], "error", $name);
                return false;
            }
            ...
            if ($obj->isRunonEnd()) {
                ...
                if (isset($post[$name]) or isset($_FILES[$name])) {
                    ...
                    if (is_null($value) and (bool) $field['required'] === true) {
                        ...
                        \core\Notify::add("Вы не заполнили поле " . $field['title'], "error", $field['name']);
                        return false;
                    } else {
                        if (is_null($value)) {
                            $value = "";
        ...
        \core\Notify::add("Ты олень! нельзя тебе форму отправлять", "error", "olen");
    ...
    public function send() {
        ...
        $form_before_send = new \adminmenu\events\FormBeforeSend($form, null);
        ...
        $form_after_send = new \adminmenu\events\FormAfterSend($form, null);
        event($form_before_send);
    ...
    private function sendNotifyOnAdminEmail() {
        \mailer\models\Tomail::send($this->form['email'], "Новое сообщение из " . $this->form['title'], $this->notify_message);
    }
