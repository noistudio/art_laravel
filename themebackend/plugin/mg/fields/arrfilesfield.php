...
if (isset($value) and is_array($value) and count($value)) {
    foreach ($value as $val) {
        ...
        <p>
            <a href="#" data-name="<?php echo $name; ?>" class="choose_file btn btn-success">Выбрать файл</a>
            <input class="choose_value_<?php echo $name; ?>" type="hidden" name="<?php echo $name; ?>[]" value="<?php echo $val['url']; ?>">

            <a target="_blank" href="<?php echo $val['url']; ?>" class="namefile choose_file_<?php echo $name; ?> btn btn-warning"><?= $val['name'] ?></a>
            <a href="#" class="deleteblock btn btn-danger">[x]</a>
        </p>
        <?php
        ?>
