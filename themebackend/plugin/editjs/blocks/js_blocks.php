...
if (isset($block['vars']) and is_array($block['vars']) and count($block['vars']) > 0) {
    foreach ($block['vars'] as $key => $var) {
        ...
        var tmp_el="<input class='form-control' type='text' value='"+val_tmp+"' placeholder='<?php echo $var; ?>' id='<?php echo $name_var; ?>'>"
    ...
    if (isset($block['texts']) and count($block['texts'])) {
        foreach ($block['texts'] as $key => $var) {
            ...
            var tmp_el="<div class='cdx-input cdx-warning__message textarea-cdx' contenteditable='true' data-placeholder='<?php echo $var; ?>'  id='<?php echo $name_var; ?>' spellcheck='false' data-gramm='false'>"+val_tmp+"</div>";
            div.innerHTML=div.innerHTML+tmp_el;
            <?php
        }
    }
    ...
    */
    ...
    var texts = toolsContent.querySelectorAll('.textarea-cdx');
    if(texts.length>0){
    for (var i = 0; i < texts.length; ++i) {
    console.log(texts[i]);
    result[texts[i].id]=texts[i].innerHTML;
