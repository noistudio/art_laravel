...
Vvveb.baseUrl="/themeadmin/name/builder_js/";
		...
		blocksList.each(function ()
		{
			...
			for (group in Vvveb.BlocksGroup)	
			{
				...
				for (i in blocks)
				{
					...
					{
                                            if(block.id){
						item = $('<li data-section="' + group + '" data-drag-type=block data-type="' + blockType + '" data-search="' + block.name.toLowerCase() + '"><a href="#">' + block.name + "</a></li><li><a class='btn btn-danger' target='_blank' href='"+$("#pathadmin").data("path")+"blocks/update/"+block.id+"'>Настройка</a></li>");
                                            }else {
...
//				});
    ...
    _frameLoaded : function() {
		...
		self.frameHead.append('<link data-vvveb-helpers href="/themeadmin/name/builder_js/css/vvvebjs-editor-helpers.css" rel="stylesheet">');
	...
	saveAjax: function(fileName, startTemplateUrl, callback)
	{
		...
		if (!startTemplateUrl || startTemplateUrl == null)
		{
			data["html"] = this.getHtml();
                ...
                data['html']='<div class="container " id="editableContent">'+$(window.FrameDocument).find("#editableContent").html()+"</div>";
var path_admin=$("#pathadmin").data("path");
    path_admin=path_admin+"builder/save/index";
    var csrf_val=$('meta[name=csrf-token]').attr("content");
    data["_csrf"]=csrf_val;
		$.ajax({
			type: "POST",
			url: path_admin,//set your server side save script url
			data: data,
			cache: false,
